...
def review_paper_with_refiner(
    ...
    **pipeline_kwargs
    ...
    # Get paper insight miner output
    insight_miner_json = None
    if paper_insight_miner and content:
        if verbose:
            print("Getting paper insight miner output...")
        try:
            # Format initial review as text for the miner
            initial_review_text = refiner._format_review_dict(initial_review, review_format) if hasattr(refiner, '_format_review_dict') else str(initial_review)
            insight_miner_json = paper_insight_miner.mine_paper_insights(content, initial_review_text)
        except Exception as e:
    ...
    # Get related work JSON list
    related_work_json_list_str = None
    search_keywords = None
    if related_work_searcher:
        if verbose:
            print("Getting related work JSON list...")
        try:
            import json
            ...
            # Convert to JSON string for the refiner prompt
            related_work_json_list_str = json.dumps(related_work_json_list, ensure_ascii=False)
            # Also capture the search keywords used internally, if available
